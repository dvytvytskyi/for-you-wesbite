...
export async function buildMainSitemapXml(): Promise<string> {
  const staticRoutes = [
    ...
    { path: '/projects', changeFreq: 'weekly' as const, priority: 0.7 },
  ...
  try {
    ...
    // Keep sitemap generation resilient when API is temporarily unavailable.
  }

  try {
    const developersResult = await getDevelopers({ summary: true, page: 1, limit: 1000 });
    (developersResult.developers || []).forEach((dev) => {
      if (!dev?.id) return;
      entries.push(
        buildUrlEntry(`/developers/${dev.id}`, {
...
export async function buildProjectsSitemapXml(startIndex: number, endExclusive?: number): Promise<string> {
  // Properties URLs are intentionally excluded from sitemap to preserve crawl budget.
  // Keep this endpoint alive for backward compatibility, but return an empty urlset.
  return wrapUrlSet([]);
